...
import { storage, users, databases } from "../../app/api/appwrite.config";
...
export const createProduct = async ({
  ...
  try {
    ...
    console.log(product)
    const newProduct = await databases.createDocument(process.env.NEXT_PUBLIC_DATABASE_ID, process.env.NEXT_PUBLIC_PRODUCT_COLLECTION_ID, ID.unique(), {
...
export const deleteProductById = async(productId) => {
  try{
    const product = await databases.deleteDocument(process.env.NEXT_PUBLIC_DATABASE_ID, process.env.NEXT_PUBLIC_PRODUCT_COLLECTION_ID, productId);
    return parseStringify(product);
  }catch(error){
    console.log(error);
  }
